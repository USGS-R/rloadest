...
\parskip 3pt
%\VignetteIndexEntry{Example Application 5}
...
app5.ld <- predLoad(app5.lrR2, app5.est, by="day", 
                    load.units="pounds")
# Get the 1999 data and merge to get daily flows and append 
# load, note that the units must match what was dselected for
